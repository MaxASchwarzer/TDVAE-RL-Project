...
class ConvPreProcess(nn.Module):
    ...
    def __init__(self, input_size, d_hidden, d_out):
        super().__init__()
        self.resnet = ops.ResNet(3, [(2, d_hidden, 1), (2, d_hidden * 2, 2), (2, d_hidden * 4, 2),
                                     (2, d_hidden * 8, 2)], norm=nn.BatchNorm2d)
        self.fc = nn.Linear((d_hidden * 8) * (input_size[1] // 8) ** 2, d_out)

    def forward(self, x):
        bs, seq_len = x.size()[:2]
        x = F.elu(self.resnet(x.view(-1, *x.size()[2:])))
        x = x.view(bs * seq_len, -1)
        return self.fc(x).view(bs, seq_len, -1)


class ConvDecoder(nn.Module):
    """ The pre-process layer for image."""

    def __init__(self, z_size, d_hidden, input_size):
        super().__init__()
        self.init_size = (d_hidden * 8, input_size[1] // 8, input_size[1] // 8)
        init_size_flat = np.prod(self.init_size)
        self.fc = nn.Linear(z_size, init_size_flat)
        self.bn = nn.BatchNorm1d(init_size_flat)
        self.resnet = ops.ResNet(self.init_size[0], [(2, d_hidden * 8, 1), (2, d_hidden * 4, -2), (2, d_hidden * 2, -2),
                                                     (2, d_hidden, -2), (1, 3, 1)],
                                 norm=nn.BatchNorm2d, skip_last_norm=True)

    def forward(self, x):
        bs, seq_len = x.size()[:2]
        x = F.elu(self.bn(self.fc(x))).view(-1, *self.init_size)
        x = self.resnet(x)
        return torch.sigmoid(x.view(bs, seq_len, -1))
